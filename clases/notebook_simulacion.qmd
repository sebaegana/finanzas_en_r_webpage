---
listing_title: "Clase 04 - Notebook Simulaciones"
title: "Finanzas en R"
...
Pensemos en un activo, que desconocemos su precio futuro. Una forma de estimarlo es generar mÃºltiples caminos que podrÃ­a seguir el precio del activo en base a ciertos supuestos.

Se puede asumir que el precio de una acciÃ³n sigue un paseo aleatorio, representado por un proceso Browniano geomÃ©trico como el siguiente:

$$ dS = \mu Sdt + \sigma Sdw$$

donde $S$ corresponde al precio de un activo (por ejemplo, una acciÃ³n), $\mu$ corresponde al valor esperado de los retornos (drift) y $\sigma$ corresponde a la volatilidad (volatility), $dw$ es el diferencial de un proceso de Wiener, que posee las siguientes propiedades:

1.  $dw = \epsilon \sqrt{dt}$, donde $\epsilon \sim N(0,1)$
2.  Los cambios de $dw$ son independientes

En tÃ©rminos discretos:

$$ S_{t + 1} = S_{t} * exp((\mu - \frac{\sigma^2}{2})\Delta t + \sigma \epsilon_{t} \sqrt{\Delta t})$$ Definimos los valores para un activo cualquiera, que posee un valor de 100 en el perÃ­odo t (hoy o en el perÃ­odo 0):
...
## Simulaciones y valoraciÃ³n de opciones

### Black & Scholes

Para valorar opciones, se utiliza la formula de Black & Scholes:

$$ C = S * N(d_1) - X * exp (-rT) * N(d_2)$$ $$ P = X * exp (-rT) * N(-d_2) - S * N(-d_1)$$ donde $C$ corresponde al precio de una opciÃ³n de compra, $P$ corresponde al precio de una opciÃ³n de venta, $X$ es el precio strike, $S$ corresponde al precio actual del activo, $T$ es el Time to madurity. Por otra parte:
...
Utilizamos la misma expresiÃ³n discreta:
...
# AplicaciÃ³n en evaluaciÃ³n de proyectos

En el caso de proyectos, podemos realizar un anÃ¡lisis de carÃ¡cter estocÃ¡stico, lo que implica asumir que alguna de las variables no es conocida pero si que puede estar situada dentro de algunos parÃ¡metros.

Veamos el siguiente ejemplo:

Una empresa estÃ¡ evaluando la introducciÃ³n de un nuevo producto y desea conocer la probabilidad que obtenga pÃ©rdidas. Para cumplir dicho propÃ³sito lo contrata a usted para construir un modelo financiero del negocio.

DespuÃ©s de realizar la debida investigaciÃ³n, usted decide construir una simulaciÃ³n de Monte Carlo que le permita generar la distribuciÃ³n de probabilidad de las utilidades del negocio, modelando separadamente los ingresos y los costos totales, considerando los siguientes elementos:

-   Por el lado de los ingresos totales, se considerarÃ¡n tres escenarios (A, B y C) para el precio y la cantidad, los cuales aparecen descritos en la siguiente tabla:
...
-   En el caso de los costos totales, los costos fijos son iguales a \$150 y los costos variables unitarios son constantes y son modelados usando una distribuciÃ³n triangular con los siguientes parÃ¡metros:
...
A. Â¿CuÃ¡l es la probabilidad que el negocio tenga pÃ©rdidas?
